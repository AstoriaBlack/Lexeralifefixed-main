...
const SpellingGame = () => {
      ...
      try {
        ...
        for (const word of fetchedWords) {
          ...
          try {
            ...
            assets[word.word] = { sound }
          } catch (error) {
            console.warn(`Could not preload sound for ${word.word}:`, error)
          }
        }
    ...
    try {
      ...
      if (user) {
        ...
        if (userDoc.exists()) {
          ...
          await setDoc(userDocRef, userProgress)
        }
      }
    } catch (error) {
  ...
  }, [loading, words, currentWord]) // Added missing dependency: currentWord
  ...
  const getLocalWordData = () => {
    return [
      ...
      },
      {
        ...
        image: "https://source.unsplash.com/300x200/?apple",
        sound: "apple.mp3",
        difficulty: "medium",
      },
      {
        ...
        <View style={styles.progressBar}>
          <View
            style={[
              styles.progressFill,
              {
                width: `${(usedWords.length / (words.filter((w) => w.difficulty === currentLevel).length || 1)) * 100}%`,
              },
            ]}
          />
        </View>
